...
3. 死锁位置： 每次获取锁就将当前的行号，线程id，以及能记录位置的信息，放入一个队列，释放的时候就出队列。如果有死锁，可以额外开个线程去查看，就是队列里面剩下的元素。
...
- ```c++
  ...
  int main() {
      ...
      MyObject* obj1 = pool.allocate(); // 从内存池中分配对象
      new (obj1) MyObject(10); // 使用 placement new 构造对象
  
      MyObject* obj2 = pool.allocate();
      new (obj2) MyObject(20);
  
      obj1->~MyObject(); // 手动调用析构函数
      pool.deallocate(obj1); // 将对象归还到内存池
  ...
  //总结，内存池设计需要用到的数据结构（简易）  一个vector记录所有的内存块，定义一个空闲内存块结构体，一个空闲内存链表用于保存空闲内存
  //构造时就可以先初始化一个内存块，记录在vector中，然后均分内存块为固定大小，记录在空闲内存链表
  //使用时从空闲链表中取一个节点即可，如果空闲内存链表用完了，就在开辟一块内存块并记录在vector，和均分内存块记录在空闲内存链表中，这里只有增加，可以扩充内存如果缩容
...
- ```c++
  ...
  public:
      ...
      auto enqueue(F&& f, Args&&... args) -> std::future<typename std::result_of<F(Args...)>::type> {
          ...
          // 将任务封装为 std::packaged_task，以便获取返回值
