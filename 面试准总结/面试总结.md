...
`epoll` 是 Linux 内核提供的一种高效的 I/O 事件通知机制，用于处理大量并发连接。它是对传统 `select` 和 `poll` 的改进，特别适合高并发场景

##### **1. 核心特点**

1. 高效性：
   - `epoll` 使用事件驱动机制，只关注活跃的文件描述符，避免了轮询所有文件描述符的开销。
   - 支持水平触发（LT）和边缘触发（ET）两种模式，适应不同性能需求。
2. 可扩展性：
   - 可以处理成千上万的并发连接，性能不会随文件描述符数量增加而显著下降。
3. 灵活性：
   - 支持动态添加、修改和删除文件描述符及其关注的事件。
...
1. 红黑树：
   - 用于存储所有注册的文件描述符及其关注的事件，支持高效的增删改查操作。
2. 就绪队列：
   - 当文件描述符上的事件发生时，内核会将其加入就绪队列，`epoll_wait` 只需遍历该队列即可获取就绪事件。

###### **触发模式**

1. 水平触发（LT）：
   - 默认模式，只要文件描述符处于就绪状态，`epoll_wait` 就会持续通知。
   - 适合简单场景，编程模型更直观。
2. 边缘触发（ET）：
   - 仅在文件描述符状态发生变化时通知一次，适合高性能场景。
   - 需要用户程序确保一次性处理完所有数据，否则可能丢失事件。
