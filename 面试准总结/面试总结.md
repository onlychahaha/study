...
[C++ STL 无序容器存储状态示意图](./images/media/1-200221131A4220.png)
...
- 等待队列：在内核表现为一个链表的数据结构，存放着的是被阻塞的进程。epoll_wait未检测到事件，就会将当前进程加入到等待队列，并使其进入阻塞状态，让cpu去做其他事情，当有事件来了，内核会通过回调唤醒被阻塞的进程。
  ...
  整体流程：先用epoll_create创建一个红黑树，再通过epoll_ctl将文件描述符插入到红黑树中，进行监测，当有对应的fd事件来了，红黑树会通过回调将就绪的任务存放在就绪队列中，再epoll_wait调用期间，内核会扫描就绪队列，如果有任务了，会将再等待队列的进程唤醒，并且内核将就绪队列的事件分批次拷贝到用户态，然后用户再去处理对应事件
...
1.**网卡接受数据包：**网卡接受到数据包会进行帧校验和mac地址匹配，都通过了的数据包会被加入到网卡接受缓冲区

2.**内核对数据包的处理：**网卡驱动通过dma（直接内存访问）将数据包从网卡缓冲区拷贝到内核环形缓冲区，然后网卡触发中断，通知cpu将数据包封装sk_buf结构，并加入到cpu处理队列中

3**.内核协议栈解封数据包：**软中断触发，内核协议栈开始处理数据包，根据数据包的类型，协议栈开始解封装，并通过路由表，iptable等工具进一步处理，最终，内核将数据包传给对应的socket，将其放到对应的socket接受队列

|  **场景**  |             **有协议栈**             |          **无协议栈**          |
| :--------: | :----------------------------------: | :----------------------------: |
|  数据解析  | 自动分层解析，应用层直接获取有效数据 | 需手动解析原始比特流，极易出错 |
| 跨网络通信 |        通过IP路由实现全球互联        |        仅限直连设备通信        |
|  错误处理  |       CRC校验、TCP重传自动修复       |     数据损坏或丢失无法恢复     |
|  开发效率  |  标准化接口（如Socket API）简化开发  |     需从头实现所有协议逻辑     |

回溯通过dpdk抓包过后，是自己再用户态上实现了部分的协议栈，就是只是解析协议，但不像完整的协议栈，比如数据包tcp重传，完整的协议栈还会去解决tcp重传。

4.用户从socket接受到数据，开始处理
...
eg：![img](./images/media/image2021-7-17_10-34-35.png)
...
![image-20250225145249689](./images/media/image-20250225145249689.png)
...
​		![image-20250225145408996](./images/media/image-20250225145408996.png)
...
![img](./images/media/wps2.png)

修改后的流程：![img](./images/media/wps3.png)

配置的框架：

​					![多进程配置流程](./images/media/多进程配置流程.png)

## 常用组件

### git工作流程

![img](./images/media/84a7a87c9b738213fb4aaf1604d8f889.png)
